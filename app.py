...
def get_info_from_total_db(mun_id, year):
    if year == '2025':
        Totals_query = Totals_2025
    elif year == '2021':
    ...
    try:
        ...
        # Consultar por municipio y año
        cursor.execute(Totals_query, (mun_id,))
        row = cursor.fetchone()
        conn.close()
...
def get_info_from_db(section_id, year):
    if year == '2025':
        ...
        Map_query = Map_query_casillas_2017
    try:
        conn = sqlite3.connect(DB_PATH)
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
...
def get_section_info(mun_id,section_id):
    ...
    votos_2017 = get_info_from_db(section_id,'2017')
    data_2025 = jsonify({"2025" :votos_2025, "2021" :votos_2021, "2017":votos_2017})
    print (mun_id)
    return data_2025
